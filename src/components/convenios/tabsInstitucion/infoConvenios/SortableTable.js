...
const SortableTable = ({ data, columns, setConvenio }) => {
  ...
  return (
    <div className="overflow-x-auto">
        ...
        <tbody>
          {sortedData.map((row, rowIndex) => (
            <tr className="border-b-2 border-gris-300" key={rowIndex}>
              <td className="px-6 py-4 whitespace-nowrap w-[20px]">
                {row["indice"]}
              </td>
              <td className={"px-6 py-4 whitespace-nowrap"}>
                <span
                  onClick={(e) => handleClick(row["indice"] - 1)}
                  className="cursor-pointer anchor-ver-convenio"
                >
                  {row["nombre"]}
                </span>
              </td>
              ...
              <td className={"px-6 py-4 whitespace-nowrap"}>
                {row["textoResolucion"] === null ? (
                  "Sin resolución"
                ) : (
                  <a
                    href={row["linkResolucion"]}
                    target="_blank"
      ...
      <Tooltip
        key="tooltipVerResolucion"
        anchorSelect=".anchor-ver-resolucion"
        place="top"
      >
        Haga click para ver resolución
      </Tooltip>
      <Tooltip
        key="tooltipVerConvenio"
        anchorSelect=".anchor-ver-convenio"
        place="top"
      >
        Haga click para ver detalle
      </Tooltip>
    </div>
  );
};
