...
function Extracciones({ 
    ...
    const validateForm = () => {
        ...
        const documentoWordValid = documento_word !== null;
        ...
        const allValidations = Object.values({
            ...
            documento_word: documentoWordValid,
    ...
    const handleFileChange = (e, fieldName) => {
        ...
        if (file) {
            if (fieldName === 'documento_word') {
                // Para documento Word, aceptar archivos .doc y .docx
                ...
                if (allowedTypes.includes(file.type) || allowedExtensions.includes(fileExtension)) {
                    ...
                    alert('Por favor seleccione un archivo de Word (.doc o .docx)');
                    e.target.value = '';
                }
            } else {
                // Para otros archivos, validar que sea archivo comprimido
    ...
    const onSubmit = (e) => {
        ...
        if (validateForm()) {
            ...
            submitFormData.append('documento_word', documento_word);
               ...
               <div className="flex flex-row">
                  ...
                  {user && (user.is_pc_isla_admin || user.is_pc_isla_editor) && (
                    <>
                      ...
                      <label className="text-gris-700 text-sm w-[80px] mr-2 text-center">Doc. Word</label>
            ...
            (user.is_pc_isla_admin || user.is_pc_isla_editor) && (
                ...
                <div className="mt-4">
                    ...
                    <form onSubmit={e => {onSubmit(e)}} method="POST" action="#">
                        ...
                        <div className="grid grid-cols-1 sm-sii:grid-cols-3 gap-4 mt-3">
                            ...
                            {/* Documento Word */}
                            <div>
                                <label className="text-gris-700 text-sm">Documento Word (DOC/DOCX):</label>
                                <input 
                                    type="file"
                                    id="documento_word"
                                    name="documento_word"
                                    onChange={e => handleFileChange(e, 'documento_word')}
                                    accept=".doc,.docx"
                                    required
                                    className="block w-full rounded border border-azul-marino-100 bg-clip-padding px-3 py-2 text-gris-800 transition file:-mx-3 file:-my-[0.32rem] file:overflow-hidden file:rounded-none file:border-0 file:border-solid file:cursor-pointer file:border-inherit file:bg-gris-800 file:px-3 file:py-[0.32rem] file:text-white file:transition file:duration-150 file:ease-in-out file:[border-inline-end-width:1px] file:[margin-inline-end:0.75rem] hover:file:bg-gris-400 hover:file:text-gris-800 focus:border-primary focus:text-gris-800 focus:shadow-te-primary focus:outline-none"
                                />
                                <span className="text-rojo-400 text-sm" hidden={validations.documento_word}>
                                    Debe adjuntar el documento Word.
                                </span>
                            </div>
                        </div>
